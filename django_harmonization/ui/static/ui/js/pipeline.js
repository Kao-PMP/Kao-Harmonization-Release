...
var createPipelineModule = function() {
    ...
    function get_fetch_url_promise(url) {
        var dict = { 
            headers:{ 'Accept':'application/json', 'X-Requested-With':'XMLHttpRequest',
                'Authorization':'Token fa392aa886cad716b3e32e02256d4ef0e80b3080' }  // TODO don't hardcode the security token
        };
        var p = fetch(url, dict).then( async function(response) { 
                if (response) {
                    if (!response.ok) { 
                        ...
                        console.log("pipeline.get_fetch_url_promise() Error 1: (no value) XXX")
    ...
    async function _start_pipeline() {
        ...
        dbname="heart_db"  // TODO hard-coded db name!
